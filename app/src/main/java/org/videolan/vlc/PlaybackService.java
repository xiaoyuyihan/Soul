...
import com.example.soul.PlayVideoActivity.AudioUtil;
import com.example.soul.PlayVideoActivity.VideoPlayerActivity;
...
public class PlaybackService extends Service implements IVLCVout.Callback {
   ...
   // private PopupManager mPopupManager;
    ...
    private static LibVLC LibVLC() {
        return VLCInstance.get(mAppContext);
    }
    ...
    private final MediaPlayer.EventListener mMediaPlayerListener = new MediaPlayer.EventListener() {
        KeyguardManager keyguardManager = (KeyguardManager) mAppContext.getSystemService(Context.KEYGUARD_SERVICE);
    ...
    public boolean switchToVideo() {
        ...
        if (isVideoPlaying()) {//Player is already running, just send it an intent
            ...
            VideoPlayerActivity.startOpened(mAppContext,media.getUri(), mCurrentIndex);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_TOAST:
                    ...
                    Toast.makeText(mAppContext, text, duration).show();
                    break;
            }
        }
    }
    ...
    private void showNotification() {
        ...
        try {
            ...
            if (cover == null)
                cover = BitmapFactory.decodeResource(mAppContext.getResources(), R.mipmap.ic_launcher);
    ...
    public synchronized void loadLastPlaylist(int type) {
        ...
        if (time > 0)
            seek(time);
        if (!audio) {
            boolean paused = mSettings.getBoolean(VIDEO_PAUSED, !isPlaying());
            float rate = mSettings.getFloat(VIDEO_SPEED, getRate());
            if (paused)
                pause();
            if (rate != 1.0f)
                setRate(rate, false);
    ...
    private synchronized void savePosition(){
        ...
        if(!audio) {
            editor.putBoolean(VIDEO_PAUSED, !isPlaying());
            editor.putFloat(VIDEO_SPEED, getRate());
        }
        Util.commitPreferences(editor);
    }
    ...
    public String getArtistPrev() {
        if (mPrevIndex != -1)
            return MediaUtils.getMediaArtist(this, mMediaList.getMedia(mPrevIndex));
        else
            return null;
    }
    ...
    public String getArtistNext() {
        if (mNextIndex != -1)
            return MediaUtils.getMediaArtist(this, mMediaList.getMedia(mNextIndex));
        else
            return null;
    }
    ...
    public Bitmap getCoverPrev() {
        if (mPrevIndex != -1)
            return AudioUtil.getCover(this, mMediaList.getMedia(mPrevIndex), 64);
        else
            return null;
    }
    ...
    public Bitmap getCoverNext() {
        if (mNextIndex != -1)
            return AudioUtil.getCover(this, mMediaList.getMedia(mNextIndex), 64);
        else
            return null;
    }
    ...
    public void playIndex(int index, int flags) {
        ...
        final Media media = new Media(VLCInstance.get(mAppContext), mw.getUri());
        ...
        if (mw .getType() != MediaWrapper.TYPE_VIDEO || mw.hasFlag(MediaWrapper.MEDIA_FORCE_AUDIO)
            ...
            if (!isVideoPlaying && mMediaPlayer.getRate() == 1.0F )
                setRate(1.0F, true);
            mMediaPlayer.play();
           ...
           VideoPlayerActivity.startOpened(mAppContext,getCurrentMediaWrapper().getUri(), mCurrentIndex);
        }
    }
...
*/
    ...
    public static class Client {
        ...
        @MainThread
        public interface Callback {
            void onConnected(org.videolan.vlc.PlaybackService service);
            void onDisconnected();
        }
        ...
        private final ServiceConnection mServiceConnection = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder iBinder) {
                ...
                final org.videolan.vlc.PlaybackService service = getService(iBinder);
                if (service != null)
        ...
        private static Intent getServiceIntent(Context context) {
            return new Intent(context, org.videolan.vlc.PlaybackService.class);
        }
